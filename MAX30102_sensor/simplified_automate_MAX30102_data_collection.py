...
serial_port = 'COM6'  # Adjust as necessary
baud_rate = 115200
...
def start_recording():
    ...
    csv_writer.writerow(["Time", "Red_light", "IR_light", "HR", "HRvalid", "SpO2", "SpO2Valid"])
    csv_file.flush()
...
def read_data():
    ...
    if ser.in_waiting > 0:
        ...
        try:
            values = line.split(",")
            if len(values) == 6:
                red, ir, hr, HRvalid, SpO2, SpO2valid = [float(v.split("=")[1]) for v in values]
                ...
                if recording and csv_writer:
                    csv_writer.writerow([time.time(), red, ir, hr, HRvalid, SpO2, SpO2valid])
                    csv_file.flush()
            else:
